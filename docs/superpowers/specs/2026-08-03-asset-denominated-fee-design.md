...
5. **Cost-basis imprecision for base-asset-fee buys (accepted trade-off).** Netting the fee into `quantity` fixes the *position* (the primary reported bug) but leaves `get_economic_basis` (`realized.py`) computing `basis = net_qty × fill_price`, which is `|fee| × fill_price` LESS than the cash actually paid (`gross_qty × fill_price`). Consequently realized gains on disposal are overstated by `|fee| × fill_price` per lot, and the IRR path's per-trade outflow (`_calculate_cash_flow` = `net_qty × price`) is understated by the same amount. The fee's value is captured on the asset side (the holding is smaller) but not itemized on the cash/cost side — so the portfolio balances, but the fee is an un-itemized realized cost. Magnitude is small (largest in the user's data: ~$3.48 on a ~$3478 trade; most are sub-cent). This is an accepted trade-off for keeping the protected calc layer untouched; the textbook-correct alternative (fold the fee into basis via calc-layer edits) was rejected for violating the no-calc-layer constraint. An alternative that WOULD make position, cash_flow, basis, and IRR all self-consistent with zero calc changes — persisting `price = cash_paid / net_qty` instead of the raw fill price — was considered and deferred: it would diverge the displayed fill price from the exchange's reported price, which the user has not requested. Revisit if basis accuracy becomes material; the full fee-accounting redesign (issues #28/#30's eventual model) is the right place.
