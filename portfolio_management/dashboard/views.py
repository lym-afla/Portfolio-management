...
from common.models import FX, Brokers, Transactions
...
def dashboard(request):
    ...
    analysis = NAV_at_date(
        ...
        currency_target,
        ["Asset type", "Currency", "Asset class"],
    )
    ...
    try:
        summary["Return"] = format_percentage(
            (summary["NAV"] - summary["Cash-out"]) / summary["Invested"] - 1, digits=1
        )
    except Exception:
        summary["Return"] = "–"
    ...
    summary["Invested"] = currency_format(
        summary["Invested"], currency_target, number_of_digits
    )
    summary["Cash-out"] = currency_format(
        summary["Cash-out"], currency_target, number_of_digits
    )
    ...
    financial_table_context = dashboard_summary_over_time(
        user, effective_current_date, user.custom_brokers, currency_target
    )
    # Formatting outputs
    for index in range(len(financial_table_context["lines"])):
        if financial_table_context["lines"][index]["name"] == "TSR":
            for k, v in financial_table_context["lines"][index]["data"].items():
                financial_table_context["lines"][index]["data"][k] = format_percentage(
                    v, digits=1
                )
        else:
            financial_table_context["lines"][index] = currency_format_dict_values(
                financial_table_context["lines"][index],
                currency_target,
                number_of_digits,
            )
    ...
    chart_settings["To"] = get_last_exit_date_for_brokers(
        selected_brokers, effective_current_date
    )
    from_date = calculate_from_date(chart_settings["To"], chart_settings["timeline"])
    if from_date == "1900-01-01":
        from_date = (
            Transactions.objects.filter(investor=user, broker__in=selected_brokers)
            .order_by("date")
            .first()
            .date
        )
