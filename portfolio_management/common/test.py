...
class AssetsBuyInPriceTestCase(TestCase):
    def setUp(self):
        ...
        # Create FX rates
        FX.objects.create(
            date=date(2023, 1, 1), investor=self.user, USDEUR=Decimal("1.1")
        )
        FX.objects.create(
            date=date(2023, 3, 1), investor=self.user, USDEUR=Decimal("1.15")
        )
        FX.objects.create(
            date=date(2023, 5, 1), investor=self.user, USDEUR=Decimal("1.25")
        )
    ...
    def test_calculate_buy_in_price_with_broker_filter(self):
        ...
        another_broker = Brokers.objects.create(
            investor=self.user, name="Another Broker"
        )
        Transactions.objects.create(
            investor=self.user,
        ...
        buy_in_price = self.asset.calculate_buy_in_price(
            date(2023, 6, 15), broker_id_list=[self.broker.id]
        )
        self.assertAlmostEqual(buy_in_price, Decimal("6.1364"), places=4)
    ...
    def test_calculate_buy_in_price_with_start_date(self):
        ...
        buy_in_price = self.asset.calculate_buy_in_price(
            date(2023, 7, 23), start_date=date(2023, 5, 1)
        )
        # Expected: (4*2 + 7*4) / 6
