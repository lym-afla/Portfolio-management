...
class FX(models.Model):
    ...
    investor = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="FX"
    )
    ...
    def get_rate(cls, source, target, date):
        ...
        pairs_list = [
            ...
            if (field.name != "date" and field.name != "id")
        ]
        ...
        for i in range(1, len(cross_currency)):
            ...
            for element in pairs_list:
                if i_source in element and i_target in element:
                    ...
                    fx_call = (
                        cls.objects.filter(
                            date__lte=date, **{f"{field_name}__isnull": False}
                        )
                    ...
                    if fx_call is None or fx_call["quote"] is None:
                        fx_call = (
                            cls.objects.filter(
                                date__gte=date, **{f"{field_name}__isnull": False}
                            )
                            .values("date", quote=F(field_name))
                            .order_by("date")
                            .first()
                        ...
                        if fx_call is None or fx_call["quote"] is None:
                            raise ValueError(
                                f"No FX rate found for {field_name} after before {date}"
                            )
    ...
    def update_fx_rate(cls, date, investor):
        ...
        fx_variables = [
            ...
            if (field.name != "date" and field.name != "investor")
        ]
        ...
        try:
            fx_instance, created = cls.objects.get_or_create(
            ...
            if existing_rate is None:
                ...
                try:
                    ...
                    if rate_data is not None:
                        ...
                        print(
                            "FX model, update FX. line 125",
                            rate_data["requested_date"],
                        ...
                        print(
                            f'{source}{target} for {rate_data["requested_date"]} is updated'
                        )
                except Exception:
                    print(
                        f"{source}{target} for {date} is NOT updated. "
                        f"Yahoo Finance is not responding correctly"
                    )
                    continue
...
class Brokers(models.Model):
    ...
    def balance(self, date):
        ...
        # This approach in order to match how balances are calculated in
        # 'transactions' app after each transaction
        transactions = self.transactions.filter(date__lte=date)
        for transaction in transactions:
            balance[transaction.currency] = balance.get(
                transaction.currency, Decimal(0)
            ) - Decimal(
                (transaction.price or 0) * Decimal(transaction.quantity or 0)
        ...
        for fx_transaction in fx_transactions:
            balance[fx_transaction.from_currency] = (
                balance.get(fx_transaction.from_currency, Decimal(0))
                - fx_transaction.from_amount
            )
            balance[fx_transaction.to_currency] = (
                balance.get(fx_transaction.to_currency, Decimal(0))
                + fx_transaction.to_amount
            )
            if fx_transaction.commission:
                commission_currency = (
                    fx_transaction.from_currency
                )  # Assume commission is in the source currency
                balance[commission_currency] = (
                    balance.get(commission_currency, Decimal(0))
...
class Assets(models.Model):
    ...
    def price_at_date(self, price_date, currency=None):
        try:
            ...
            if currency is not None:
                quote.price = (
                    quote.price * FX.get_rate(self.currency, currency, price_date)["FX"]
                )
            return quote
        except Exception:
            return None
    ...
    def exit_dates(self, end_date, broker_id_list=None, start_date=None):
        ...
        transactions = self.transactions.filter(
            date__lte=end_date, quantity__isnull=False
        )
        if broker_id_list is not None:
            transactions = transactions.filter(broker_id__in=broker_id_list)
    ...
    def calculate_buy_in_price(
        self, date, currency=None, broker_id_list=None, start_date=None
    ):
        """
        ...
        Args:
            ...
            broker_id_list (Optional[List[str]]): A list of broker IDs to filter the
                transactions by. Defaults to None.
            start_date (Optional[datetime.date]): The start date for the calculation.
                Defaults to None.
        ...
        transactions = self.transactions.filter(
            quantity__isnull=False, date__lte=date
        ).order_by(
            "date"
    ...
    def realized_gain_loss(
        ...
        Parameters:
            ...
            broker_id_list (list, optional): A list of broker IDs to filter the
                transactions. Defaults to None.
            start_date (datetime.date, optional): The start date for the calculation.
                Defaults to None.
        ...
        if len(self.exit_dates(date, broker_id_list)) != 0:
            ...
            transactions_before_entry = self.transactions.filter(
                date__lte=latest_exit_date, quantity__isnull=False
            )
            if start_date is not None:
                transactions_before_entry = transactions_before_entry.filter(
                    date__gte=start_date
                )
            if broker_id_list is not None:
                transactions_before_entry = transactions_before_entry.filter(
                    broker_id__in=broker_id_list
                )
            ...
            if len(transactions_before_entry) != 0:
                if currency is not None:
                    for transaction in transactions_before_entry:
                        ...
                        if fx_rate:
                            total_gl_before_current_position -= (
                                transaction.price * transaction.quantity * fx_rate
                            )
                    if start_date is not None:
                        total_gl_before_current_position -= self.price_at_date(
                            start_date, currency
                        ).price * self.position(start_date)
                else:
                    total_gl_before_current_position = (
                        transactions_before_entry.aggregate(
        ...
        if position_at_date != 0:
            ...
            if broker_id_list is not None:
                exit_transactions = exit_transactions.filter(
                    broker_id__in=broker_id_list
                )

            for exit in exit_transactions:
                buy_in_price = self.calculate_buy_in_price(
                    exit.date, exit.currency, broker_id_list, start_date
                )
                if buy_in_price is not None:
                    if currency is not None:
                        ...
                        fx_rate = 1
                    if fx_rate:
                        realized_gain_loss_for_current_position -= (
                            (exit.price - buy_in_price) * fx_rate * (exit.quantity)
                        )
                else:
        ...
        return {
            "current_position": round(
                Decimal(realized_gain_loss_for_current_position), 2
            ),
            "all_time": round(
                Decimal(
                    total_gl_before_current_position
    ...
    def unrealized_gain_loss(
        ...
        Parameters:
            ...
            currency (str, optional): The currency in which the capital distribution
                is calculated.
                If None, the original currency of the dividends is used.
                Defaults to None.
            broker_id_list (list, optional): A list of broker IDs to filter the
                transactions. Defaults to None.
            start_date (datetime.date, optional): The start date for the calculation.
        ...
        Returns:
            float: The total capital distribution (dividends) for the security as of the given date,
            rounded to 2 decimal places. If there are no dividend transactions, it returns 0.

        This method calculates the capital distribution by summing up the cash flow
        from all 'dividend' type transactions as of the given date. If a currency is
        provided, the dividends are converted to that currency using the FX.get_rate
        function.
        """
        ...
        buy_in_price = self.calculate_buy_in_price(
            date, currency, broker_id_list, start_date
        )
        if buy_in_price is not None:
    ...
    def get_capital_distribution(
        ...
        dividend_transactions = self.transactions.filter(
            type="Dividend", date__lte=date
        )

        if broker_id_list is not None:
            dividend_transactions = dividend_transactions.filter(
                broker_id__in=broker_id_list
            )
        ...
        if dividend_transactions:
            if currency is None:
                total_dividends += dividend_transactions.aggregate(
                    total=Sum("cash_flow")
                )["total"]
            else:
                for dividend in dividend_transactions:
                    fx_rate = FX.get_rate(dividend.currency, currency, dividend.date)[
                        "FX"
                    ]
                    if fx_rate:
    ...
    def get_commission(self, date, currency=None, broker_id_list=None, start_date=None):
        ...
        commission_transactions = self.transactions.filter(
            commission__isnull=False, date__lte=date
        )
...
class Transactions(models.Model):
    ...
    broker = models.ForeignKey(
        Brokers, on_delete=models.CASCADE, related_name="transactions"
    )
...
def update_FX_from_Yahoo(base_currency, target_currency, date, max_attempts=5):
    ...
    while attempt < max_attempts:
        ...
        try:
            exchange_rate_data = data.history(
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=models.Q(broker__isnull=False)
                | models.Q(broker_group__isnull=False),
                name="either_broker_or_group",
            ),
            models.CheckConstraint(
                check=~(
                    models.Q(broker__isnull=False)
                    & models.Q(broker_group__isnull=False)
                ),
...
class FXTransaction(models.Model):
    ...
    broker = models.ForeignKey(
        Brokers, on_delete=models.CASCADE, related_name="fx_transactions"
    )
    date = models.DateField(null=False)
    ...
    exchange_rate = models.DecimalField(
        max_digits=15, decimal_places=6, null=False, blank=True
    )
    commission = models.DecimalField(
        max_digits=15, decimal_places=6, null=True, blank=True
    )
