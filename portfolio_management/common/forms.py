...
class DashboardForm(forms.ModelForm):
    custom_brokers = forms.ChoiceField(
        ...
        widget=GroupedSelect(
            attrs={"class": "form-select", "id": "inputDashboardBrokers"}
        ),
        label="Brokers",
    )
    ...
    table_date = forms.DateField(
        widget=forms.DateInput(
            attrs={"class": "form-control", "id": "inputTableDate", "type": "date"}
        ),
        label="Date",
    )
    ...
    class Meta:
        ...
        widgets = {
            "default_currency": forms.Select(
                attrs={"class": "form-select", "id": "inputCurrency"}
            ),
            "digits": forms.NumberInput(
                attrs={"class": "form-control", "id": "numberOfDigits"}
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user is not None:
            ...
            all_choices = [
                choice[0]
                for group, choices in broker_choices
                for choice in choices
                if choices != "__SEPARATOR__"
            ]
            if initial_value in all_choices:
                self.fields["custom_brokers"].initial = initial_value
            else:
                logger.warning(
                    f"Dashboard form. Initial value '{initial_value}' "
                    "not found in choices. Defaulting to 'All'."
                )
