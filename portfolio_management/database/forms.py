...
class TransactionForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if investor is not None:
            self.fields["broker"].choices = [
                (broker.pk, broker.name)
                for broker in Brokers.objects.filter(investor=investor).order_by("name")
            ]
            security_choices = [
                (security.pk, security.name)
                for security in Assets.objects.filter(investor=investor).order_by(
                    "name"
                )
            ]
            ...
            self.fields["broker"].choices = [
                (broker.pk, broker.name)
                for broker in Brokers.objects.order_by("name").all()
            ]

        self.fields["type"].choices = [
            (choice[0], choice[0])
            for choice in Transactions._meta.get_field("type").choices
            if choice[0]
        ]
...
class PriceForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["security"].choices = [
            (security.pk, security.name)
            for security in Assets.objects.filter(investor=investor)
            .order_by("name")
            .all()
        ]
...
class SecurityForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["custom_brokers"].choices = [
        ...
        if self.instance.pk:
            self.fields["custom_brokers"].initial = [
                broker.id for broker in self.instance.brokers.all()
            ]
...
class BrokerPerformanceForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        broker_or_group_choices.append(
            ("Broker Groups", tuple((group, group) for group in BROKER_GROUPS.keys()))
        )
        ...
        if from_amount and to_amount and exchange_rate:
            ...
            if abs(calculated_rate - exchange_rate) > 0.0001:
                raise forms.ValidationError(
                    "Exchange rate does not match the provided amounts."
                )
...
class PriceImportForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        if user:
            self.fields["securities"].queryset = Assets.objects.filter(
                investor=user
            ).order_by("name")
            self.fields["broker"].queryset = Brokers.objects.filter(
                investor=user
            ).order_by("name")
