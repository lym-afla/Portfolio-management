...
from utils import (
    ...
    broker_group_to_ids,
    currency_format,
    currency_format_dict_values,
    format_percentage,
    get_last_exit_date_for_brokers,
    parse_broker_cash_flows,
    parse_excel_file_transactions,
...
def database_brokers(request):
    ...
    for broker in brokers:
        ...
        broker.NAV = NAV_at_date(
            user.id, [broker.id], effective_current_date, currency_target, []
        )["Total NAV"]

        try:
            broker.first_investment = (
                Transactions.objects.filter(investor=user, broker=broker)
                .order_by("date")
                .first()
                .date
            )
    ...
    broker_totals = currency_format_dict_values(
        broker_totals, currency_target, number_of_digits
    )
    broker_totals["IRR"] = format_percentage(
        Irr(
            user.id,
            effective_current_date,
            currency_target,
...
def database_securities(request):
    ...
    for security in securities:
        ...
        if security.price_at_date(effective_current_date) is not None:
            security.current_value = currency_format(
            ...
            security.irr = format_percentage(
                Irr(
                    user.id,
                    effective_current_date,
                    security.currency,
        ...
        security.realised = currency_format(
            security.realized_gain_loss(effective_current_date)["all_time"],
            security.currency,
            number_of_digits,
        )
        security.unrealised = currency_format(
            security.unrealized_gain_loss(effective_current_date),
            security.currency,
            number_of_digits,
        )
        security.capital_distribution = currency_format(
            security.get_capital_distribution(effective_current_date),
            security.currency,
            number_of_digits,
        )
...
def get_price_data_for_table(request):
    ...
    order_by = (
        F(columns[order_column]).desc(nulls_last=True)
        if order_dir == "desc"
        else F(columns[order_column]).asc(nulls_last=True)
    )
    ...
    if search_value:
        query &= (
            Q(security__name__icontains=search_value)
        ...
        data.append(
            {
                "id": element.id,
                "date": element.date,
                "security": element.security.name,
                "asset_type": element.security.get_type_display(),
                "currency": element.security.currency,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            fx_instance, created = FX.objects.get_or_create(
                date=fx_date, investor=request.user, defaults={}
            )
            ...
            if currency_pair in pairs_list:
                ...
                if current_rate is None:
                    ...
                    logger.info(
                        f"Existing FX rate found for {currency_pair} on {fx_date}. "
                        "Keeping existing rate."
                    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.headers.get("x-requested-with") == "XMLHttpRequest":
                # If it's an AJAX request, return a JSON response with success and redirect_url
                return JsonResponse(
                    {"success": True, "redirect_url": reverse("database:brokers")}
                )
            else:
                # If it's not an AJAX request, redirect to a URL
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.headers.get("x-requested-with") == "XMLHttpRequest":
                # If it's an AJAX request, return a JSON response with success and redirect_url
                return JsonResponse(
                    {"success": True, "redirect_url": reverse("database:prices")}
                )
            else:
                # If it's not an AJAX request, redirect to a URL
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                return JsonResponse(
                    {"status": "exists", "message": "Security already exists."}
                )
            ...
            if request.headers.get("x-requested-with") == "XMLHttpRequest":
                ...
                if import_flag:
                    return JsonResponse(
                        {"status": "success", "message": "Security added successfully."}
                    )
                else:
                    return JsonResponse(
                        {
                            ...
                            "redirect_url": reverse("database:securities"),
                        }
                    )
            else:
                # If it's not an AJAX request, redirect to a URL
...
def import_transactions_form(request):
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        brokers = list(
            Brokers.objects.filter(investor=request.user).values("id", "name")
        )
        return JsonResponse(
            {
                "brokers": brokers,
...
def import_transactions(request):
    if request.method == "POST":
        ...
        if import_type == "transaction":
            securities, transactions = parse_excel_file_transactions(
                file, currency, broker_id
            )

            for security in securities:
                if not Assets.objects.filter(
                    investor=user, name=security["name"], ISIN=security["isin"]
                ).exists():
                    return JsonResponse(
                        {"status": "missing_security", "security": security}
                    )
        ...
        if quantity is not None or request.POST.get("type") == "Dividend":
            try:
                security = Assets.objects.get(
                    name=request.POST.get("security_name"), investor=request.user
                )
            except Assets.DoesNotExist:
                return JsonResponse(
                    {
                        "status": "error",
                        "errors": {"security_name": ["Security not found"]},
                    },
                    status=400,
                )
        else:
...
def get_update_fx_dates(request):
    if request.method == "GET":
        dates = (
            Transactions.objects.filter(investor=request.user)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if is_restricted_str == "None":
                is_restricted_list = [
                    None
                ]  # This will be used to indicate both restricted and unrestricted
                ...
                return JsonResponse(
                    {"error": 'Invalid "is_restricted" value'}, status=400
                )
            ...
            def generate_progress():
                ...
                for curr in currencies:
                    for is_restricted in is_restricted_list:
                        total_operations += get_years_count(
                            user,
                            effective_current_date,
                            broker_or_group,
                            curr,
                ...
                try:
                    for curr in currencies:
                        for is_restricted in is_restricted_list:
                            for (
                                progress_data
                            ) in save_or_update_annual_broker_performance(
                                user,
                                effective_current_date,
                                broker_or_group,
                                curr,
...
def get_years_count(
    user, effective_date, brokers_or_group, currency_target, is_restricted
):
        ...
        if form.is_valid():
            ...
            def generate_progress():
                ...
                for i, security in enumerate(securities, 1):
                    try:
                        security = Assets.objects.get(
                            id=security.id, investor=request.user
                        )
                        ...
                        if security.data_source == "FT" and security.update_link:
                            ...
                            current_operation += len(
                                dates
                            )  # Skip all dates for this security
                            continue
...
def generate_dates(start, end, frequency):
    ...
    if frequency == "monthly":
        ...
        current = date(start.year, quarter_end_month, 1) + relativedelta(
            months=1, days=-1
        )
...
def import_security_prices_from_ft(security, dates):
    ...
    if elem and "data-mod-config" in elem.attrs:
        ...
        for d in dates:
            ...
            if Prices.objects.filter(security=security, date=d).exists():
                yield result
                continue
            ...
            try:
                r = requests.get(
                    f"https://markets.ft.com/data/equities/ajax/get-historical-prices?startDate={start_date}&endDate={end_date}&symbol={xid}",  # noqa: E501
                    headers=headers,
                    timeout=10,
                )
                r.raise_for_status()
                ...
                if not df.empty:
                    ...
                    Prices.objects.create(security=security, date=d, price=latest_price)
...
def import_security_prices_from_yahoo(security, dates):
    ...
    for d in dates:
        ...
        try:
            ...
            logger.error(
                f"Network error while fetching data for {security.name}: {str(e)}"
            )
            result["status"] = "error"
