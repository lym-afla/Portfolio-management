...
def _get_cash_balances_for_api(
    user: CustomUser, target_date: date, selected_account_ids: List[int] = None
) -> Dict[str, str]:
    """
    ...
    """
    selected_account_ids = selected_account_ids or get_selected_account_ids(
        user, user.selected_account_type, user.selected_account_id
    )
    logger.debug(
...
def _filter_assets(
    user: CustomUser, end_date: date, selected_account_ids: List[int], is_closed: bool, search: str
) -> QuerySet[Assets]:
    """
