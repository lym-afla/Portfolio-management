...
from common.models import FX, AnnualPerformance, Assets, Transactions, Accounts, Brokers
...
@lru_cache(maxsize=None)
def NAV_at_date(user_id: int, account_ids: Tuple[int], date: date, target_currency: str, breakdown: Tuple[str] = ()) -> Dict:
    ...
    for security in portfolio:
        ...
        if security_price is not None:
            ...
            security_price = security.calculate_buy_in_price(date, user_id, target_currency, account_ids)
...
def _calculate_portfolio_value(user_id: int, date: date, currency: Optional[str] = None, 
                            asset_id: Optional[int] = None, account_ids: Optional[List[int]] = None) -> Decimal:
    if asset_id is None:
...
def calculate_portfolio_cash(user_id: int, account_ids: List[int], date: date, target_currency: str) -> Decimal:
    """
...
def IRR(user_id: int, date: date, currency: Optional[str] = None, asset_id: Optional[int] = None, 
        account_ids: Optional[List[int]] = None, start_date: Optional[date] = None, cached_nav: Optional[Decimal] = None) -> Union[Decimal, str]:
    """
    ...
    if start_date is not None:
        ...
        start_portfolio_value = _calculate_portfolio_value(user_id, initial_value_date, currency, asset_id, account_ids)
...
def get_selected_account_ids(user: CustomUser, selection_type: str, selection_id: Optional[int] = None) -> List[int]:
    ...
    if selection_type == 'all':
        return list(Accounts.objects.filter(
            broker__investor=user
        ).values_list('id', flat=True))
    
    elif selection_type == 'account':
        return [selection_id] if Accounts.objects.filter(
            id=selection_id,
            broker__investor=user
        ).exists() else []
...
def calculate_performance(user, start_date, end_date, account_group_type, account_group_id, currency_target, is_restricted=None):
    ...
    accounts = Accounts.objects.filter(id__in=selected_account_ids, broker__investor=user).select_related('broker')
    ...
    for account in accounts:
        ...
        for asset in assets:
            ...
            performance_data['capital_distribution'] += asset.get_capital_distribution(end_date, user, currency_target, account_ids=[account.id], start_date=start_date)
    ...
    # Calculate TSR
    performance_data['tsr'] = format_percentage(IRR(user.id, end_date, currency_target, account_ids=selected_account_ids, start_date=start_date), digits=1)
...
def get_last_exit_date_for_accounts(account_ids: List[int], effective_current_date: date) -> Optional[date]:
    """
