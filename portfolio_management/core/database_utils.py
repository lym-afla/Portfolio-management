...
async def save_or_update_annual_broker_performance(
    user,
    effective_date,
    account_group_type,
    account_group_id,
    ...
    selected_account_ids = await database_sync_to_async(get_selected_account_ids)(
    ...
    last_exit_date = await database_sync_to_async(get_last_exit_date_for_accounts)(
    ...
    logger.info(
        f"Calculating performance for {len(years)} years. "
        f"First transaction: {first_transaction.date}, "
...
async def save_annual_performance(
    user,
    account_group_type,
    account_group_id,
    year,
