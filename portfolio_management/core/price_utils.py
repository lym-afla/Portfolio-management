...
def _filter_prices(
    user, start_date, end_date, selected_asset_types, selected_account, selected_securities, search
):
    """
    ...
    if search:
        prices_query = prices_query.filter(
            Q(security__name__icontains=search) | Q(security__type__icontains=search)
        )
