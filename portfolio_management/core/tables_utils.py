...
def _calculate_closed_table_output_for_api(
    ...
    for asset in portfolio:
        ...
        for i, exit_date in enumerate(exit_dates):
            ...
            # of same asset as separate rows
            position = {
                "id": asset.id,
                "type": asset.type,
                "name": asset.name,
            ...
            for transaction in entry_transactions:
                fx_rate = (
                    get_fx_rate(transaction.currency, currency_used, transaction.date)
                    if currency_used
                    else 1
                )
    ...
    if portfolio_closed_totals.get("entry_value", Decimal(0)) != 0:
        portfolio_closed_totals["price_change_percentage"] = (
            portfolio_closed_totals.get("realized_gl", Decimal(0))
            + portfolio_closed_totals.get("unrealized_gl", Decimal(0))
...
def _calculate_open_table_output_for_api(
    ...
    total_irr_start_date = (
        start_date  # Not to be overwritten by asset start date if start date is not defined
    )
            ...
            if not use_default_currency:
                ...
                if key == "entry_value":
                    ...
                    addition = asset.realized_gain_loss(
                        end_date, user_id, currency_target, selected_account_ids, asset_start_date
                    )["current_position"]["total"]
    ...
    if "entry_value" in portfolio_open_totals and portfolio_open_totals["entry_value"] != 0:
        ...
        if "capital_distribution" in categories:
            portfolio_open_totals["capital_distribution_percentage"] = (
                portfolio_open_totals["capital_distribution"] / abs_entry_value
            )

        if "commission" in categories:
            portfolio_open_totals["commission_percentage"] = (
                portfolio_open_totals["commission"] / abs_entry_value
            )

        portfolio_open_totals["total_return_percentage"] = (
            portfolio_open_totals["total_return_amount"] / abs_entry_value
        )
        portfolio_open_totals["share_of_portfolio"] = (
            portfolio_open_totals["current_value"] / portfolio_NAV
        )
    ...
    print(
        f"Total processing time for calculate_open_table_output_for_api: "
