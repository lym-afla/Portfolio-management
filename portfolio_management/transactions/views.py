...
def transactions(request):
    ...
    all_transactions = sorted(
        chain(transactions, fx_transactions), key=attrgetter("date")
    )
    ...
    for transaction in all_transactions:
        ...
        if isinstance(transaction, Transactions):
            ...
            if transaction.cash_flow:
                transaction.cash_flow = currency_format(
                    transaction.cash_flow, transaction.currency, number_of_digits
                )
            if transaction.commission:
                transaction.commission = currency_format(
            ...
            transaction.from_amount = currency_format(
                -transaction.from_amount, transaction.from_currency, number_of_digits
            )
            transaction.to_amount = currency_format(
                transaction.to_amount, transaction.to_currency, number_of_digits
            )
            if transaction.commission:
                transaction.commission = currency_format(
                    -transaction.commission, transaction.from_currency, number_of_digits
                )
