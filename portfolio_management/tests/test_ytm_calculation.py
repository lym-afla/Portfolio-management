...
from common.models import Accounts, Assets, BondCouponSchedule, BondMetadata, Brokers, Transactions
...
class YTMCalculationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = CustomUser.objects.create_user(
            username="testuser", email="test@example.com", password="testpass123", digits=2
        )
    ...
    def test_ytm_calculation_basic(self):
        ...
        self.assertLess(
            abs(float(ytm)), 100, "YTM should be reasonable (absolute value less than 100%)"
        )
    ...
    def test_compare_with_existing_method(self):
        ...
        if ytm_new is not None and ytm_old is not None:
            ...
            self.assertLess(
                difference, 0.01, f"YTM results should be close (difference: {difference:.4f}%)"
            )
