...
class TestAssetsModel:
    ...
    def test_realized_gain_loss_complex_scenario(
        ...
        for (
            ...
            ), f"For date {test_date}: Expected gain/loss to be "
            ...
            ), f"For date {test_date}: Expected gain/loss to be {expected_gain_loss_all_time}, "
    ...
    def test_calculate_buy_in_price(self, user, account, asset, general_transactions, caplog):
        ...
        print(f"Asset transactions: {asset.transactions.all()}")
        ...
        for test_date, expected_price in test_dates:
            ...
            assert (
                buy_in_price == expected_price
            ), f"For date {test_date}: Expected buy-in price to be {expected_price}, "
    ...
    def test_calculate_buy_in_price_with_start_date(
        ...
        result = asset.calculate_buy_in_price(
            end_date, user, currency="USD", account_ids=[account.id], start_date=start_date
        )
    ...
    def test_calculate_buy_in_price_with_FX(self, user, asset, caplog, general_transactions):
        ...
        for test_date, expected_price in test_dates:
            ...
            assert round(buy_in_price, 4) == round(
                expected_price, 4
            ), f"For date {test_date}: Expected buy-in price to be {expected_price}, "
            f"but got {buy_in_price}"
    ...
    def test_unrealized_gain_loss_with_currency_conversion_and_start_date(
        ...
        expected_gain = (
            (Decimal("120") / Decimal("1.25")) - (Decimal("110") / Decimal("1.05"))
        ) * Decimal("10")
        assert result["total"] == pytest.approx(expected_gain, rel=Decimal("1e-2"))
...
class TestRealizedGainLoss:
    ...
    def test_same_day_open_close(self, caplog):
        ...
        print("\nResult:", result)
        expected_total = (
            Decimal("50") + Decimal("25") + Decimal("16")
        )  # (105-100)*10 + (115-110)*5 + (114-112)*8
        assert result["all_time"]["total"] == expected_total
        assert result["current_position"]["total"] == Decimal("0")
