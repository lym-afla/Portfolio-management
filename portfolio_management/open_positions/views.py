...
def update_open_positions_table(request):
    ...
    portfolio_open = Assets.objects.filter(
        investor=user, transactions__broker_id__in=selected_brokers
    )
    ...
    portfolio_open = (
        portfolio_open.filter(
            transactions__date__lte=end_date,
        )
        .prefetch_related("transactions")
        .annotate(abs_total_quantity=Abs(Sum("transactions__quantity")))
        .exclude(abs_total_quantity__lt=TOLERANCE)
    )
...
def get_cash_balances(user, timespan, effective_current_date):
    ...
    serializable_balances = {
        currency_format("", currency, 0): currency_format(
            balance, currency, number_of_digits
        )
