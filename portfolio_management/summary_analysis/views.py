...
from common.models import AnnualPerformance, Assets, Brokers, Transactions
from utils import (
    broker_group_to_ids,
    brokers_summary_data,
    currency_format,
    format_percentage,
    get_fx_rate,
...
def summary_view(request):
    ...
    broker_table_contexts = brokers_summary_data(
        user, effective_current_date, user_brokers, currency_target, number_of_digits
    )

    first_year = (
        AnnualPerformance.objects.filter(investor=user, broker__in=user_brokers)
        .order_by("year")
        .first()
        .year
    )
    last_exit_date = get_last_exit_date_for_brokers(
        [broker.id for broker in user_brokers], effective_current_date
    )
    last_year = (
        last_exit_date.year
    ...
    context = {
        ...
        "buttons": buttons,
        "unrestricted_investments_context": broker_table_contexts[
            "public_markets_context"
        ],
        "restricted_investments_context": broker_table_contexts[
...
def exposure_table_update(request):
    ...
    for asset in assets:
        ...
        asset.entry_price = asset.calculate_buy_in_price(
            end_date, currency_target, broker_ids, start_date
        ) or Decimal(0)
        ...
        for asset_category, values in data[category].items():
            if asset_category == "Cash":
                ...
                line = {
                    ...
                    "market_value": values["market_value"],
                    "portfolio_percent": 0,  # Calculated after summing up all market values
    ...
    for category in categories:
        ...
        for line in context[f"{category.lower()}_context"]:
            line["portfolio_percent"] = format_percentage(
                line["market_value"] / total_market_value if total_market_value else 0,
                digits=1,
            )
    ...
    for category in categories:
        total_line = {
            "name": "TOTAL",
            "cost": currency_format(
                totals[category]["cost"], currency_target, number_of_digits
            ),
            "unrealized": currency_format(
                totals[category]["unrealized"], currency_target, number_of_digits
            ),
            "unrealized_percent": format_percentage(
                (
                    totals[category]["unrealized"] / totals[category]["cost"]
                    if totals[category]["cost"]
                    else 0
                ),
                digits=1,
            ),
            "market_value": currency_format(
                totals[category]["market_value"], currency_target, number_of_digits
            ),
            "portfolio_percent": "",
            "realized": currency_format(
                totals[category]["realized"], currency_target, number_of_digits
            ),
            "realized_percent": format_percentage(
                (
                    totals[category]["realized"] / totals[category]["cost"]
                    if totals[category]["cost"]
                    else 0
                ),
                digits=1,
            ...
            "capital_distribution": currency_format(
                totals[category]["capital_distribution"],
                currency_target,
                number_of_digits,
            ),
            "capital_distribution_percent": format_percentage(
                (
                    totals[category]["capital_distribution"] / totals[category]["cost"]
                    if totals[category]["cost"]
                    else 0
                ),
                digits=1,
            ...
            "commission": currency_format(
                totals[category]["commission"], currency_target, number_of_digits
            ),
            "commission_percent": format_percentage(
                (
                    totals[category]["commission"] / totals[category]["cost"]
                    if totals[category]["cost"]
                    else 0
                ),
                digits=1,
            ...
            "total": currency_format(
                totals[category]["unrealized"]
                + totals[category]["realized"]
                + totals[category]["capital_distribution"]
                + totals[category]["commission"],
                currency_target,
                number_of_digits,
            ),
            "total_percent": format_percentage(
...
def categorize_asset(asset):
    if asset.exposure == "Equity":
        ...
        return (
            "Fixed income - RU" if asset.currency == "RUB" else "Fixed income - Int'l"
        )
