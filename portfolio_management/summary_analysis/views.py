...
class SummaryViewSet(viewsets.ViewSet):
    ...
    def get_exposure_table_data(self, user, start_date, end_date, currency_target, number_of_digits):
        ...
        for account in accounts:
            ...
            for transaction in commission_transactions:
                fx_rate = get_fx_rate(transaction['currency'], currency_target, transaction['date'], user)
    ...
    def prepare_total_line(self, totals, currency_target, number_of_digits):
        return {
            ...
            'unrealized_percent': format_percentage(totals['unrealized'] / totals['cost'] if totals['cost'] else 0, digits=1),
            'market_value': currency_format(totals['market_value'], currency_target, number_of_digits),
            'portfolio_percent': '',
            'realized': currency_format(totals['realized'], currency_target, number_of_digits),
            'realized_percent': format_percentage(totals['realized'] / totals['cost'] if totals['cost'] else 0, digits=1),
            'capital_distribution': currency_format(totals['capital_distribution'], currency_target, number_of_digits),
            'capital_distribution_percent': format_percentage(totals['capital_distribution'] / totals['cost'] if totals['cost'] else 0, digits=1),
            'commission': currency_format(totals['commission'], currency_target, number_of_digits),
            'commission_percent': format_percentage(totals['commission'] / totals['cost'] if totals['cost'] else 0, digits=1),
            'total': currency_format(totals['unrealized'] + totals['realized'] + totals['capital_distribution'] +  totals['commission'], currency_target, number_of_digits),
            'total_percent': format_percentage((totals['unrealized'] + totals['realized'] + totals['capital_distribution'] - totals['commission']) / totals['cost'] if totals['cost'] else 0, digits=1),
        }