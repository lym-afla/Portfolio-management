...
class SummaryViewSet(viewsets.ViewSet):
    ...
    def get_exposure_table_data(self, user, start_date, end_date, currency_target, number_of_digits):
        ...
        for category in categories:
            data[category] = {
                ...
                'Fixed income - RU': {'cost': 0, 'unrealized': 0, 'market_value': 0, 'realized': 0, 'capital_distribution': 0, 'commission': 0},
                'Options': {'cost': 0, 'unrealized': 0, 'market_value': 0, 'realized': 0, 'capital_distribution': 0, 'commission': 0},
                'Other': {'cost': 0, 'unrealized': 0, 'market_value': 0, 'realized': 0, 'capital_distribution': 0, 'commission': 0},
                'Cash': {'market_value': 0, 'commission': 0},
            }
        ...
        for category in categories:
            for asset_category, values in data[category].items():
                line = self.prepare_line_data(asset_category, values, currency_target, number_of_digits)
                context[f'{category.lower()}_context'].append(line)
        ...
        # Adding totals to the context
        for category in categories:
            total_line = self.prepare_total_line(totals[category], currency_target, number_of_digits)
            context[f'{category.lower()}_context'].append(total_line)
    ...
    def prepare_total_line(self, totals, currency_target, number_of_digits):
        return {
            'name': 'TOTAL',
            'cost': currency_format_old_structure(totals['cost'], currency_target, number_of_digits),
            'unrealized': currency_format_old_structure(totals['unrealized'], currency_target, number_of_digits),
            'unrealized_percent': format_percentage_old_structure(totals['unrealized'] / totals['cost'] if totals['cost'] else 0, digits=1),
            'market_value': currency_format_old_structure(totals['market_value'], currency_target, number_of_digits),
            'portfolio_percent': '',
            'realized': currency_format_old_structure(totals['realized'], currency_target, number_of_digits),
            'realized_percent': format_percentage_old_structure(totals['realized'] / totals['cost'] if totals['cost'] else 0, digits=1),
            'capital_distribution': currency_format_old_structure(totals['capital_distribution'], currency_target, number_of_digits),
            'capital_distribution_percent': format_percentage_old_structure(totals['capital_distribution'] / totals['cost'] if totals['cost'] else 0, digits=1),
            'commission': currency_format_old_structure(totals['commission'], currency_target, number_of_digits),
            'commission_percent': format_percentage_old_structure(totals['commission'] / totals['cost'] if totals['cost'] else 0, digits=1),
            'total': currency_format_old_structure(totals['unrealized'] + totals['realized'] + totals['capital_distribution'] +  totals['commission'], currency_target, number_of_digits),
            'total_percent': format_percentage_old_structure((totals['unrealized'] + totals['realized'] + totals['capital_distribution'] - totals['commission']) / totals['cost'] if totals['cost'] else 0, digits=1),
        }

    # def format_portfolio_breakdown(self, context):
    #     formatted_data = []
    #     for category in ['consolidated_context', 'unrestricted_context', 'restricted_context']:
    #         for item in context[category]:
    #             formatted_data.append({
    #                 'category': f"{category.replace('_context', '').capitalize()} - {item['name']}",
    #                 'value': item['market_value'],
    #                 'percentage': item['portfolio_percent']
    #             })
