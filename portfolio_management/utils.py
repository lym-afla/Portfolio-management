...
import constants
from common.models import FX, AnnualPerformance, Assets, Brokers, Prices, Transactions
...
# func.date used for correct query when transaction is at [table_date]
# (removes time (HH:MM:SS) effectively)
def portfolio_at_date(user_id, date, brokers):
    ...
    return (
        ...
        .annotate(total_quantity=Sum("transactions__quantity"))
        .exclude(total_quantity=0)
    )


# Create one dictionary from two.
# And add values for respective keys if keys present on both dictionaries
...
def calculate_security_nav(item, date, currency):
    ...
    return round(
        Decimal(
            current_quote.price
            * get_fx_rate(item.currency.upper(), currency, current_quote.date)
            * item.position(date)
        ),
        2,
    )
...
def update_analysis(
    analysis, key, value, date=None, currency=None, desired_currency=None
):
...
def get_brokers_for_security(user_id, security_id):
    ...
    transactions = Transactions.objects.filter(
        investor__id=user_id, security_id=security_id
    )

    # Retrieve distinct brokers from the filtered transactions
    brokers = Brokers.objects.filter(
        investor__id=user_id, transactions__in=transactions
    ).distinct()
...
def NAV_at_date(
    ...
    date,
    target_currency,
    breakdown=["Asset type", "Currency", "Asset class", "Broker"],
):
    ...
    for security in portfolio:
        current_value = Decimal(
            security.position(date, broker_ids)
            * security.price_at_date(date, target_currency).price
        )
...
def Irr(
    user_id, date, currency=None, asset_id=None, broker_id_list=None, start_date=None
):
    # Calculate portfolio value
    portfolio_value = calculate_portfolio_value(
        user_id, date, currency, asset_id, broker_id_list
    )
    ...
    if start_date is not None:
        ...
        start_portfolio_value = calculate_portfolio_value(
            user_id, initial_value_date, currency, asset_id, broker_id_list
        )
        ...
        if asset_id is not None:
            ...
            if (start_portfolio_value < 0) or (
                start_portfolio_value == 0 and first_transaction < 0
            ):
                return "N/R"
    ...
    for trans in transactions:
        if trans.type == "Cash in" or trans.type == "Cash out":
            cash_flow = -1 * trans.cash_flow
        elif trans.type == "Broker commission" or trans.type == "Tax":
            cash_flow = Decimal(0)  # Do not account for pay-outs elsewhere
        else:
            cash_flow = trans.cash_flow or (
                -trans.quantity * trans.price + (trans.commission or 0)
            )

        if currency is not None:
            fx_rate = get_fx_rate(trans.currency.upper(), currency, trans.date)
        else:
            fx_rate = 1
        cash_flows.append(round(cash_flow * fx_rate, 2))
        transaction_dates.append(trans.date)
...
def calculate_portfolio_value(
    user_id, date, currency=None, asset_id=None, broker_id_list=None
):
    if asset_id is None:
        portfolio_value = NAV_at_date(user_id, broker_id_list, date, currency, [])[
            "Total NAV"
        ]
    else:
        ...
        try:
            portfolio_value = round(
                ...
                * asset.position(date, broker_id_list),
                2,
            )
        except Exception:
            portfolio_value = 0
        ...
        for key, value in data_dict[category].items():
            try:
                data_dict[percentage_key][key] = (
                    str(round(Decimal(value / total * 100), 1)) + "%"
                )
...
def get_chart_data(
    user_id, brokers, frequency, from_date, to_date, currency, breakdown
):
    # Get the correct starting date for "All time" category
    if from_date == "1900-01-01":
        from_date = (
            Transactions.objects.filter(investor__id=user_id, broker__in=brokers)
            .order_by("date")
            .first()
            .date
        )
    ...
    for d in dates:
        ...
        if breakdown in "No breakdown":
            ...
            NAV = Decimal(
                NAV_at_date(user_id, brokers, d, currency)["Total NAV"] / 1000
            )
            if previous_date is not None:
                contributions = (
                    ...
                    ).aggregate(Sum("cash_flow"))["cash_flow__sum"]
                    or 0
                )
            ...
            for key, value in NAV.items():
                index = next(
                    (
                        index
                        for (index, item) in enumerate(chart_data["datasets"])
                        if item["label"] == key
                    ),
                    None,
                )
                if index is None:
                    if len(chart_data["datasets"]) == 0 or d == dates[0]:
                        ...
                        data_entry = [
                            0 for i in range(len(chart_data["datasets"][-1]["data"]))
                        ]
...
def calculate_from_date(to_date, timeline):
    if isinstance(to_date, str):
        to_date = datetime.strptime(
            to_date, "%Y-%m-%d"
        ).date()  # Convert 'to' date to datetime.date
    ...
    if timeline == "YTD":
        ...
        from_date = datetime.strptime(
            "1900-01-01", "%Y-%m-%d"
        ).date()  # Convention that will be converted to the date of the first transaction
    else:
...
def create_price_table(security_type, user):
    ...
    selected_ids = Assets.objects.filter(type=security_type, investor=user).values_list(
        "id", flat=True
    )
    ...
    for index, item in enumerate(selected_ids):
        prices = (
            Prices.objects.filter(security_id=item)
            .order_by("date")
            .values_list("date", "price")
        )
        for d, price in prices:
            price_data[d][index] = price
            dates.add(d)
    ...
    header_row_1 = ["Date"] + [
        Assets.objects.get(id=item).name for item in selected_ids
    ]
...
def calculate_open_table_output(
    user_id,
    portfolio,
    end_date,
    categories,
    ...
    portfolio_NAV = NAV_at_date(user_id, selected_brokers, end_date, currency_target)[
        "Total NAV"
    ]
    portfolio_cash = calculate_portfolio_cash(
        user_id, selected_brokers, end_date, currency_target
    )
    ...
    for asset in portfolio:
        ...
        asset.entry_price = asset.calculate_buy_in_price(
            end_date, currency_used, selected_brokers, asset_start_date
        )

        asset.entry_value = round(asset.entry_price * asset.current_position, 2)
        asset.entry_price = currency_format(
            asset.entry_price,
            asset.currency if use_default_currency else currency_target,
            number_of_digits,
        )
        ...
        if "current_value" in categories:
            ...
            asset.share_of_portfolio = (
                asset.current_price * asset.current_position / portfolio_NAV
            )
            ...
            # Formatting
            asset.current_price = currency_format(
                asset.current_price,
                asset.currency if use_default_currency else currency_target,
                number_of_digits,
            )
        ...
        if "realized_gl" in categories:
            asset.realized_gl = asset.realized_gain_loss(
                end_date, currency_used, selected_brokers, asset_start_date
            )["current_position"]
        else:
            asset.realized_gl = 0
        ...
        if "unrealized_gl" in categories:
            asset.unrealized_gl = asset.unrealized_gain_loss(
                end_date, currency_used, selected_brokers, asset_start_date
            )
        else:
            asset.unrealized_gl = 0

        asset.price_change_percentage = (
            (asset.realized_gl + asset.unrealized_gl) / asset.entry_value
            if asset.entry_value > 0
            else "N/R"
        )

        if "capital_distribution" in categories:
            asset.capital_distribution = asset.get_capital_distribution(
                end_date, currency_used, selected_brokers, asset_start_date
            )
            asset.capital_distribution_percentage = (
                asset.capital_distribution / asset.entry_value
                if asset.entry_value > 0
                else "N/R"
            )
        else:
        ...
        if "commission" in categories:
            asset.commission = asset.get_commission(
                end_date, currency_used, selected_brokers, asset_start_date
            )
            asset.commission_percentage = (
                asset.commission / asset.entry_value if asset.entry_value > 0 else "N/R"
            )
        else:
            asset.commission = 0

        asset.total_return_amount = (
            asset.realized_gl
            + asset.unrealized_gl
            + asset.capital_distribution
            + asset.commission
        )
        asset.total_return_percentage = (
            asset.total_return_amount / asset.entry_value
            if asset.entry_value > 0
            else "N/R"
        )
        ...
        asset.irr = format_percentage(
            Irr(
                user_id,
                end_date,
                currency_used,
                asset_id=asset.id,
                broker_id_list=selected_brokers,
        ...
        for key in ["entry_value", "total_return_amount"] + list(
            ...
            if not use_default_currency:
                ...
                if key == "entry_value":
                    ...
                    asset.realized_gl = asset.realized_gain_loss(
                        end_date, currency_target, selected_brokers, asset_start_date
                    )["current_position"]
                    asset.unrealized_gl = asset.unrealized_gain_loss(
                        end_date, currency_target, selected_brokers, asset_start_date
                    )
                    ...
                    addition = (
                        asset.price_at_date(end_date, currency_target).price
        ...
        asset.entry_value = currency_format(
            asset.entry_value, currency_used, number_of_digits
        )
        asset.current_value = currency_format(
            asset.current_value, currency_used, number_of_digits
        )

        asset.realized_gl = currency_format(
            asset.realized_gl, currency_used, number_of_digits
        )

        asset.unrealized_gl = currency_format(
            asset.unrealized_gl, currency_used, number_of_digits
        )
        asset.price_change_percentage = format_percentage(asset.price_change_percentage)
        asset.capital_distribution = currency_format(
            asset.capital_distribution, currency_used, number_of_digits
        )
        asset.capital_distribution_percentage = format_percentage(
            asset.capital_distribution_percentage
        )
        asset.commission = currency_format(
            asset.commission, currency_used, number_of_digits
        )
        asset.commission_percentage = format_percentage(asset.commission_percentage)
        asset.total_return_amount = currency_format(
            asset.total_return_amount, currency_used, number_of_digits
        )
    ...
    if "entry_value" in portfolio_open_totals.keys():
        if portfolio_open_totals["entry_value"] != 0:
            portfolio_open_totals["price_change_percentage"] = (
                portfolio_open_totals.get("realized_gl", 0)
                + portfolio_open_totals.get("unrealized_gl", 0)
            ) / abs(portfolio_open_totals["entry_value"])
            if "capital_distribution" in categories:
                portfolio_open_totals["capital_distribution_percentage"] = (
                    portfolio_open_totals["capital_distribution"]
                    / portfolio_open_totals["entry_value"]
                )
            if "commission" in categories:
                portfolio_open_totals["commission_percentage"] = (
                    portfolio_open_totals["commission"]
                    / portfolio_open_totals["entry_value"]
                )
            portfolio_open_totals["total_return_percentage"] = portfolio_open_totals[
                "total_return_amount"
            ] / abs(portfolio_open_totals["entry_value"])

    portfolio_open_totals["cash"] = portfolio_cash
    portfolio_open_totals["total_nav"] = portfolio_NAV
    portfolio_open_totals["irr"] = format_percentage(
        Irr(
            user_id,
            end_date,
            currency_target,
    ...
    if portfolio_NAV == 0:
        ...
        portfolio_open_totals["cash_share_of_portfolio"] = format_percentage(
            portfolio_cash / portfolio_NAV
        )

    # Format totals
    portfolio_open_totals["all_assets_share_of_portfolio_percentage"] = (
        format_percentage(
            portfolio_open_totals["all_assets_share_of_portfolio_percentage"]
        )
    )
    portfolio_open_totals = currency_format_dict_values(
        portfolio_open_totals, currency_target, number_of_digits
    )

    return portfolio_open, portfolio_open_totals


def calculate_closed_table_output(
    user_id,
    portfolio,
    end_date,
    categories,
    ...
    for asset in portfolio:
        ...
        for i, exit_date in enumerate(exit_dates):
            ...
            next_entry_date = (
                entry_dates[entry_dates.index(entry_date) + 1]
                if entry_date in entry_dates
                and entry_dates.index(entry_date) < len(entry_dates) - 1
                else end_date
            )
            ...
            if start_date is not None:
                entry_quantity = asset.position(
                    entry_date - timedelta(days=1), selected_brokers
                )
                entry_value = (
                    asset.price_at_date(
                        entry_date - timedelta(days=1), currency_used
                    ).price
                    * entry_quantity
                )
            else:
            ...
            for trans in entry_transactions:
                fx_rate = (
                    get_fx_rate(trans.currency, currency_used, trans.date)
                    if currency_used
                    else 1
                )
                entry_value += trans.price * abs(trans.quantity) * fx_rate
                entry_quantity += abs(trans.quantity)

            position["entry_value"] = round(Decimal(entry_value), 2)

            # Calculate exit value and quantity
            exit_value = Decimal(0)
            for trans in exit_transactions:
                fx_rate = (
                    get_fx_rate(trans.currency, currency_used, trans.date)
                    if currency_used
                    else 1
                )
                exit_value += trans.price * abs(trans.quantity) * fx_rate
            ...
            position["price_change_percentage"] = (
                (position["realized_gl"]) / position["entry_value"]
                if position["entry_value"] > 0
                else "N/R"
            )

            # Calculate capital distribution including dividends
            # after exit_date but before next_entry_date
            if "capital_distribution" in categories:
                position["capital_distribution"] = round(
                    asset.get_capital_distribution(
                        exit_date, currency_used, selected_brokers, entry_date
                    )
                    + asset.get_capital_distribution(
                        next_entry_date,
                        currency_used,
                ...
                position["capital_distribution_percentage"] = (
                    Decimal(position["capital_distribution"] / position["entry_value"])
                    if position["entry_value"] > 0
                    else "N/R"
                )
            else:
            ...
            # Calculate IRR
            currency_used = asset.currency if use_default_currency else currency_target
            position["irr"] = format_percentage(
                Irr(
                    user_id,
                    exit_date,
                    currency_used,
            ...
            for key in (
                list(set(totals) & set(categories))
                + ["entry_value", "exit_value", "total_return_amount"]
                + [
                ...
                if key in position:
                    portfolio_closed_totals[key] = (
                        portfolio_closed_totals.get(key, 0) + position[key]
                    )

                    # Format position values
                    if "percentage" in key:
                        position[key] = format_percentage(
                            position[key], number_of_digits
                        )
                    else:
                        position[key] = currency_format(
                            position[key], currency_used, number_of_digits
                        )
    ...
    if (
        "entry_value" in portfolio_closed_totals
        and portfolio_closed_totals["entry_value"] != 0
    ):
        portfolio_closed_totals["price_change_percentage"] = (
            portfolio_closed_totals.get("realized_gl", 0)
            + portfolio_closed_totals.get("unrealized_gl", 0)
        ) / abs(portfolio_closed_totals["entry_value"])
        if "capital_distribution" in categories:
            portfolio_closed_totals["capital_distribution_percentage"] = (
                portfolio_closed_totals["capital_distribution"]
    ...
    portfolio_closed_totals = currency_format_dict_values(
        portfolio_closed_totals, currency_target, number_of_digits
    )
...
def parse_excel_file_transactions(file, currency, broker_id):
    ...
    while i < len(df.columns):
        if pd.notna(df.iloc[1, i]):
            ...
            isin = df.iloc[2, i]
            securities.append(
                {"name": security_name, "isin": isin, "currency": currency}
            )
...
def parse_broker_cash_flows(excel_file, currency, broker_id):
    ...
    df = pd.read_excel(
        excel_file, header=3
    )  # Line 4 has table headers, so header=3 (0-based index)
    ...
    for index, row in df.iterrows():
        ...
        for asset_name in asset_names:
            ...
            try:
                ...
                print(
                    f"Error updating price for asset {asset_name} on date {date}: {e}"
                )
    ...
    for index, row in df.iterrows():
        ...
        try:
            fx_instance, created = FX.objects.update_or_create(
                date=fx_data["date"],
                investor_id=fx_data["investor_id"],
                defaults=fx_data,
            )
            print(
                f"{'Created' if created else 'Updated'} "
                f"FX for date {date} and investor {investor_id}"
            )
        except IntegrityError as e:
...
def get_last_exit_date_for_brokers(selected_brokers, date):
    """
    Calculate the last date after which all activities ended
    and no asset was opened for the selected brokers.

    Args:
        selected_brokers (list): List of broker IDs to include in the calculation.

    Returns:
        date: The last date after which all activities ended
        and no asset was opened for the selected brokers.
    """
    ...
    # Step 2: If positions for all securities at the current date are zero,
    # find the latest transaction date
    latest_transaction_date = (
        Transactions.objects.filter(broker_id__in=selected_brokers, date__lte=date)
        .order_by("-date")
...
def broker_group_to_ids(brokers_or_group, user):
    if isinstance(brokers_or_group, str):
        if brokers_or_group == "All brokers":
            ...
            return list(
                Brokers.objects.filter(investor=user).values_list("id", flat=True)
            )
        ...
        if not set(selected_brokers).issubset(user_brokers):
            raise ValueError(
                "Some of the provided broker IDs do not belong to the user"
            )
...
def dashboard_summary_over_time(
    user, effective_date, brokers_or_group, currency_target
):
    ...
    lines["TSR"]["data"]["All-time"] = Irr(
        user.id, effective_date, currency_target, broker_id_list=selected_brokers
    )
    ...
    print(
        "Total time for summary performance calculation taken:",
...
def brokers_summary_data(
    user, effective_date, brokers_or_group, currency_target, number_of_digits
):
    ...
    start_year = first_entry.year
    last_exit_date = get_last_exit_date_for_brokers(
        selected_brokers_ids, effective_date
    )
    last_year = (
        last_exit_date.year
        if last_exit_date and last_exit_date.year < effective_date.year
    ...
    for restricted in [False, True]:
        ...
        for broker in brokers_subgroup:
            ...
            for year in ["YTD"] + years + ["All-time"]:
                ...
                line_data["data"][year] = compile_summary_data(
                    line_data["data"][year], currency_target, number_of_digits
                )
            ...
            try:
                ...
                compiled_ytd_data = compile_summary_data(
                    ytd_data, currency_target, number_of_digits
                )
            ...
            for entry in stored_data:
                if (
                    ...
                    year_data = {
                        ...
                        if k not in ("id", "broker_id", "investor_id", "broker_group")
                    }
                    formatted_year_data = compile_summary_data(
                        year_data, currency_target, number_of_digits
                    )
            ...
            if broker.restricted == restricted:
                try:
                    all_time_data["tsr"] = Irr(
                        user.id,
                        effective_date,
                        currency_target,
                        broker_id_list=[broker.id],
                    )
                except Exception as e:
                    print(
                        f"Error calculating all-time TSR for broker {broker.name}: {e}"
                    )
            ...
            formatted_all_time_data = compile_summary_data(
                all_time_data, currency_target, number_of_digits
            )
        ...
        for year in ["YTD"] + years + ["All-time"]:
            ...
            if year == "YTD":
                totals[year]["tsr"] = Irr(
                    ...
                    effective_date,
                    currency_target,
                    broker_id_list=[broker.id for broker in brokers_subgroup],
                    start_date=date(effective_date.year, 1, 1),
                )
            elif year == "All-time":
                totals[year]["tsr"] = Irr(
                    user.id,
            ...
            sub_totals_line["data"][year] = compile_summary_data(
                totals[year], currency_target, number_of_digits
            )
    ...
    for year in ["YTD"] + years + ["All-time"]:
        ...
        try:
            if year == "YTD":
                totals_line["data"][year]["tsr"] = Irr(
                    user.id,
                    effective_date,
                    currency_target,
                    broker_id_list=[broker.id for broker in brokers],
                    start_date=date(effective_date.year, 1, 1),
                )
            elif year == "All-time":
                totals_line["data"][year]["tsr"] = Irr(
        ...
        totals_line["data"][year] = compile_summary_data(
            totals_line["data"][year], currency_target, number_of_digits
        )

    # Create a new context for the total line
    total_context = {"years": ["YTD"] + years[::-1] + ["All-time"], "line": totals_line}
...
def save_or_update_annual_broker_performance(
    user,
    effective_date,
    brokers_or_group,
    currency_target,
    ...
    first_transaction = (
        Transactions.objects.filter(
            broker_id__in=selected_brokers_ids, date__lte=effective_date
        )
        .order_by("date")
        .first()
    )
    ...
    for i, year in enumerate(years, 1):
        if (
            skip_existing_years
            and AnnualPerformance.objects.filter(
                investor=user,
                broker_group=brokers_or_group,
                year=year,
                currency=currency_target,
        ...
        try:
            ...
            yield json.dumps(
                {
                    "status": "error",
                    "message": f"Error processing year {year}: {str(e)}",
                }
            ) + "\n"
...
def calculate_performance(
    ...
    for broker in brokers:
        ...
        if not bop_nav:
            bop_nav = NAV_at_date(
                user.id, [broker.id], start_date - timedelta(days=1), currency_target
            )["Total NAV"]
        else:
        ...
        if is_restricted is None:
            ...
            if broker.restricted is False:
                transactions = transactions.filter(
                    security__isnull=False, security__restricted=True
                )
            ...
            if broker.restricted is True:
                ...
                transactions = transactions.filter(
                    security__isnull=False, security__restricted=False
                ) | transactions.filter(security__isnull=True)

        transactions_df = pd.DataFrame(
            list(
                transactions.values(
                    "cash_flow", "type", "commission", "date", "currency"
                )
            )
        )
        ...
        if not transactions_df.empty:
            transactions_df["fx_rate"] = transactions_df.apply(
                lambda row: get_fx_rate(row["currency"], currency_target, row["date"]),
                axis=1,
            )

            # Calculate transaction-based metrics
            performance_data["invested"] += round(
                (
                    transactions_df[transactions_df["type"] == "Cash in"]["cash_flow"]
                    * transactions_df[transactions_df["type"] == "Cash in"]["fx_rate"]
                ).sum(),
                2,
            )
            performance_data["cash_out"] += round(
                (
                    transactions_df[transactions_df["type"] == "Cash out"]["cash_flow"]
                    * transactions_df[transactions_df["type"] == "Cash out"]["fx_rate"]
                ).sum(),
                2,
            )
            performance_data["commission"] += round(
                (transactions_df["commission"] * transactions_df["fx_rate"]).sum(), 2
            )
            performance_data["tax"] += round(
                (
                    transactions_df[transactions_df["type"] == "Tax"]["cash_flow"]
                    * transactions_df[transactions_df["type"] == "Tax"]["fx_rate"]
                ).sum(),
                2,
            )
        ...
        for asset in assets:
            ...
            performance_data["price_change"] += (
                asset_realized_gl["all_time"] if asset_realized_gl else 0
            )
            performance_data["price_change"] += asset.unrealized_gain_loss(
                end_date,
                currency_target,
                broker_id_list=[broker.id],
                start_date=start_date,
        ...
        # Calculate EOP NAV
        eop_nav = NAV_at_date(user.id, [broker.id], end_date, currency_target)[
            "Total NAV"
        ]
    ...
    performance_data["tsr"] = format_percentage(
        Irr(
            ...
            end_date,
            currency_target,
            broker_id_list=selected_brokers_ids,
            start_date=start_date,
        ),
        digits=1,
    )
...
def end_of_year_price_correction(user, year, broker_name, target_nav, asset_name):
    ...
    nav_at_end_of_year = NAV_at_date(
        user.id, [broker.id], end_of_year_date, asset.currency, []
    )["Total NAV"]
    ...
    new_price = old_price + (
        (target_nav - nav_at_end_of_year) / position_at_end_of_year
    )
    ...
    print(
        f"Old Asset Value: {result['old_asset_value']}, "
        f"New Asset Value: {result['new_asset_value']}"
    )
    ...
    if confirm.lower() == "yes":
        # Update the price
        price_instance, created = Prices.objects.get_or_create(
            security=asset, date=end_of_year_date, defaults={"price": new_price}
        )
        if not created:
...
def parse_charles_stanley_transactions(file, currency, broker_id, investor_id):
    ...
    def find_or_prompt_security(stock_description, investor, broker, security_cache):
        ...
        def find_best_matching_security(description):
            securities = Assets.objects.filter(Q(investor=investor) & Q(brokers=broker))
            security_names = [security.name for security in securities]
            best_match = process.extractOne(
                description, security_names, score_cutoff=60
            )

            if best_match:
                match_name, match_score = best_match
                print(
                    f"Potential match found: '{match_name}' (Similarity: {match_score}%)"
                )
                sys.stdout.flush()
                user_confirm = input(
                    f"Do you agree with this match for '{description}'? (yes/no/skip/exit): "
                ).lower()

                if user_confirm == "yes":
                    matched_security = securities.get(name=match_name)
                    security_cache[stock_description] = matched_security  # Update cache
                    print("Match confirmed and cached for future use.")
        ...
        if security is None:
            ...
            user_input = input(
                "Please enter the correct security name, "
                "'skip' to skip this transaction, or 'exit' to stop processing: "
            )
            ...
            if user_input.lower() == "exit":
                ...
                try:
                    ...
                    print(
                        f"No exact match found for '{user_input}'. "
    ...
    for index, row in df.iterrows():
        ...
        if pd.notna(date_str):
            ...
            if description == "* BALANCE B/F *" or description == "Cash Transfers ISA":
                ...
                or "ISA Subscription" in description
            ):
                transaction_data = {
                    "investor": investor,
                    "broker": broker,
                ...
                security = find_or_prompt_security(
                    stock_description, investor, broker, security_cache
                )
                if security is None:
                ...
                transaction_type = (
                    ...
                    if debit > 0
                    else constants.TRANSACTION_TYPE_SELL
                )
                price = round(Decimal(str(price)), price_decimal_places)
                if transaction_type == constants.TRANSACTION_TYPE_BUY:
                    quantity = round(
                        Decimal(str(debit)) / price, quantity_decimal_places
                    )
                else:
                    quantity = round(
                        -Decimal(str(credit)) / price, quantity_decimal_places
                    )
    ...
    if transactions:
        save_choice = input(
            f"Do you want to save these transactions for {broker.name}? (yes/no): "
        ).lower()
        if save_choice == "yes":
            Transactions.objects.bulk_create(
                [Transactions(**data) for data in transactions]
            )
